            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <select name="level" class="form-control @error('level') is-invalid @enderror" id="level" value="{{old('level')}}">
                        ...
                        <option value="0" {{$user->role->level == 0 ? "selected" : ""}}>Admin</option>
                        <option value="1" {{$user->role->level == 1 ? "selected" : ""}}>Teaching</option>
                        <option value="2" {{$user->role->level == 2 ? "selected" : ""}}>Non-Teaching</option>
                    </select>
                    @error('level')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                ...
                <div class="form-group">
                    ...
                    <select name="status" class="form-control @error('status') is-invalid @enderror" id="status" value="{{old('status')}}">
                        <option value=""> --- please select ---</option>
                        <option value="0" {{$user->role->status == 0 ? "selected" : ""}}>Active</option>
                        <option value="1" {{$user->role->status == 1 ? "selected" : ""}}>Inactive</option>
                        <option value="2" {{$user->role->status == 2 ? "selected" : ""}}>Inactive</option>
                    </select>
                    @error('status')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
